...
def add_chunks_to_qdrant(
    client: QdrantClient, chunks: list, username: str, embedder, doc_metadata: dict = None
):
    """
    ...
    for idx, c in enumerate(chunks):
        point_id = str(
            uuid.uuid5(uuid.NAMESPACE_DNS, f"{username}_{c['title']}_sent_{c['sent_index']}")
        )
    ...
    try:
        client.upsert(collection_name="research_papers", points=points)
    except Exception as e:
